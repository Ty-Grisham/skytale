...
func TestCryptography(t *testing.T) {
	...
	// TestGenKey will test the functions with a key generated by the GenerateKey function
	t.Run("TestGenKey", func(t *testing.T) {
		// Generate key
		genKey, err := cryptography.GenerateKey()
		if err != nil {
			t.Fatal(err)
		}

		// Encrypt data
		eData, err := cryptography.Encrypt(testData, genKey)
		if err != nil {
			t.Fatal(err)
		}

		// Decrypt data
		dData, err := cryptography.Decrypt(eData, genKey)
		if err != nil {
			t.Fatal(err)
		}
